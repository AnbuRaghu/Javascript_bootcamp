class App {
  ...
  openModal(event) {
    if (event.target.matches('.toolbar-delete')) return;  // we stop the modal to open when delete icon is clicked
  ...
  deleteNote(event) {
    event.stopPropagation();// we stop bubbling
    if (!event.target.matches('.toolbar-delete')) return;
    const id = event.target.dataset.id;
    this.notes = this.notes.filter(note => note.id !== Number(id));
    // we can also use splice() in order to delete
    this.displayNotes();
  }
